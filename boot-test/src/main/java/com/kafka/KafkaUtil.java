...
public class KafkaUtil {  
    ...
    * @param threadCount 消费线程数，该值应小于等于partition个数，多了也没用 
           ...
           executor.execute(() -> {  
                 ...
                 while (it.hasNext()){  
                    ...
                    try {  
                        ...
                        message = (Person) SerializationUtils.deserialize(bytes);  
                    } catch (Exception e) {  
                        e.printStackTrace();  
                        return;  
                    }  
    ...
    public static abstract class MqMessageHandler<T extends Serializable>{  
        public abstract void handle(Person message);  
    }  
      
      
    public static void main(String[] args) throws Exception {  
        //发送一个信息  
        send("test-kafka",new Person(20,"userName"));  
        //为test启动一个消费者，启动后每次有消息则打印对象信息  
        KafkaUtil.startConsumer("test-kafka", new MqMessageHandler<Person>() {  
            @Override  
            public void handle(Person user) {  
                //实现自己的处理逻辑，这里只打印出消息  
